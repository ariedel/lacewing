...
def traceback(argv=None):
    ...
    for i in np.arange(1,len(inputdata),1):
        ...
        try:
            ...
            ra =   float(star['RAdeg'])
            era =  float(star['e_RAdeg'])/3600000
            dec =  float(star['DEdeg'])
            edec = float(star['e_DEdeg'])/3600000
            plx = float(star['plx'])/1000.
            eplx = float(star['e_plx'])/1000.
            pmra = float(star['pmRA'])/1000.
            epmra = float(star['e_pmRA'])/1000.
            pmdec = float(star['pmDE'])/1000.
            epmdec = float(star['e_pmDE'])/1000.
            rv =   float(star['HRV'])
            erv =  float(star['e_HRV'])
        except ValueError:
            print name
            continue
...
#        ax.xaxis.set_ticks(np.arange(0,timespan,20))
        ...
        ax.text(timespan/20.,130,'Moving Group Volume-Radius',color="#FF0000")
...
#        pyplot.savefig("{3:}/traceback_{0:}_{3:}_{4:}.png".format(name,ra,dec,withmgp,method),dpi=100)
        pyplot.clf()
        pyplot.close()
